#include <stdint.h>
#include <wolfssl/wolfcrypt/pqclean/crypto_kem/ml-kem-512/clean/cbd.h>
#include <wolfssl/wolfcrypt/pqclean/crypto_kem/ml-kem-512/clean/ntt.h>
#include <wolfssl/wolfcrypt/pqclean/crypto_kem/ml-kem-512/clean/params.h>
#include <wolfssl/wolfcrypt/pqclean/crypto_kem/ml-kem-512/clean/poly.h>
#include <wolfssl/wolfcrypt/pqclean/crypto_kem/ml-kem-512/clean/reduce.h>
#include <wolfssl/wolfcrypt/pqclean/crypto_kem/ml-kem-512/clean/symmetric.h>
#include <wolfssl/wolfcrypt/pqclean/crypto_kem/ml-kem-512/clean/verify.h>
...
void PQCLEAN_MLKEM512_CLEAN_poly_frommsg(poly *r, const uint8_t msg[KYBER_INDCPA_MSGBYTES]) {
    ...
    for (i = 0; i < KYBER_N / 8; i++) {
        for (j = 0; j < 8; j++) {
            r->coeffs[8 * i + j] = 0;
            PQCLEAN_MLKEM512_CLEAN_cmov_int16(r->coeffs + 8 * i + j, ((KYBER_Q + 1) / 2),
                                              (msg[i] >> j) & 1);
        }
    }
}
 ...
 **************************************************/
void PQCLEAN_MLKEM512_CLEAN_poly_getnoise_eta1(poly *r, const uint8_t seed[KYBER_SYMBYTES],
                                               uint8_t nonce) {
 ...
 *              - uint8_t nonce: one-byte input nonce
 **************************************************/
void PQCLEAN_MLKEM512_CLEAN_poly_getnoise_eta2(poly *r, const uint8_t seed[KYBER_SYMBYTES],
                                               uint8_t nonce) {
...
void PQCLEAN_MLKEM512_CLEAN_poly_basemul_montgomery(poly *r, const poly *a, const poly *b) {
    ...
    for (i = 0; i < KYBER_N / 4; i++) {
        PQCLEAN_MLKEM512_CLEAN_basemul(&r->coeffs[4 * i], &a->coeffs[4 * i], &b->coeffs[4 * i],
                                       PQCLEAN_MLKEM512_CLEAN_zetas[64 + i]);
        PQCLEAN_MLKEM512_CLEAN_basemul(&r->coeffs[4 * i + 2], &a->coeffs[4 * i + 2],
                                       &b->coeffs[4 * i + 2],
                                       -PQCLEAN_MLKEM512_CLEAN_zetas[64 + i]);
    }
}
