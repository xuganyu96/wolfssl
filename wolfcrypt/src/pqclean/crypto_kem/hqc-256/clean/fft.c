 ...
 * @param[in] m_f 2^{m_f} is the smallest power of 2 greater or equal to the number of coefficients
 * of f
 */
 ...
 */
static void fft_rec(uint16_t *w, uint16_t *f, size_t f_coeffs, uint8_t m, uint32_t m_f,
    ...
    for (i = 0; i + 1 < m; ++i) {
        gammas[i] =
            PQCLEAN_HQC256_CLEAN_gf_mul(betas[i], PQCLEAN_HQC256_CLEAN_gf_inverse(betas[m - 1]));
 ...
 * Note that on this first call (as opposed to the recursive calls to fft_rec), gammas are equal to
 * betas, meaning the first gammas subset sums are actually the subset sums of betas (except 1).
 * <br> Also note that f is altered during computation (twisted at each level).
 *
 * @param[out] w Array
...
/**
 * @brief Retrieves the error polynomial error from the evaluations w of the ELP (Error Locator
 * Polynomial) on all field elements.
 *
