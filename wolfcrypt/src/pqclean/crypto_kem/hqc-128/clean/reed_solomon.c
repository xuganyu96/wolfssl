 ...
 *
 * This is a constant time implementation of Berlekamp's simplified algorithm (see @cite
 * lin1983error (Chapter 6 - BCH Codes). <br> We use the letter p for rho which is initialized at
 * -1. <br> The array X_sigma_p represents the polynomial X^(mu-rho)*sigma_p(X). <br> Instead of
 * maintaining a list of sigmas, we update in place both sigma and X_sigma_p. <br> sigma_copy serves
 * as a temporary save of sigma in case X_sigma_p needs to be updated. <br> We can properly correct
 * only if the degree of sigma does not exceed PARAM_DELTA. This means only the first PARAM_DELTA +
 * 1 coefficients of sigma are of value and we only need to save its first PARAM_DELTA - 1
 * coefficients.
 *
 ...
 * @param[out] error_compact Array of PARAM_DELTA + PARAM_N1 elements receiving a compact
 * representation of the vector error
 ...
 * @param[in] error_compact Array of PARAM_DELTA + PARAM_N1 storing compact representation of the
 * error
 */
static void compute_error_values(uint16_t *error_values, const uint16_t *z, const uint8_t *error) {
    ...
    for (size_t i = 0; i < PARAM_DELTA; ++i) {
        ...
        for (size_t k = 1; k < PARAM_DELTA; ++k) {
            tmp2 = PQCLEAN_HQC128_CLEAN_gf_mul(
                tmp2, (1 ^ PQCLEAN_HQC128_CLEAN_gf_mul(inverse, beta_j[(i + k) % PARAM_DELTA])));
        }
 ...
 *    <li> The third step, done by additive FFT, is finding the error-locator numbers by calculating
 * the roots of the polynomial sigma and takings their inverses. <li> The fourth step, is the
 * polynomial z(x). <li> The fifth step, is the computation of the error values. <li> The sixth step
 * is the correction of the errors in the received polynomial.
 *    </ol>
