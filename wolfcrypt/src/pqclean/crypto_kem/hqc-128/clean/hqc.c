 ...
 *
 * The public key is composed of the syndrome <b>s</b> as well as the <b>seed</b> used to generate
 * the vector <b>h</b>.
 *
 ...
 */
void PQCLEAN_HQC128_CLEAN_hqc_pke_keygen(uint8_t *pk, uint8_t *sk, WC_RNG *rng) {
    ...
    wc_RNG_GenerateBlock(rng, sk_seed, SEED_BYTES);
    wc_RNG_GenerateBlock(rng, sigma, VEC_K_SIZE_BYTES);
    PQCLEAN_HQC128_CLEAN_seedexpander_init(&sk_seedexpander, sk_seed, SEED_BYTES);

    wc_RNG_GenerateBlock(rng, pk_seed, SEED_BYTES);
 ...
 */
void PQCLEAN_HQC128_CLEAN_hqc_pke_encrypt(uint64_t *u, uint64_t *v, uint8_t *m, uint8_t *theta,
                                          const uint8_t *pk) {
 ...
 * @returns 0
 */
uint8_t PQCLEAN_HQC128_CLEAN_hqc_pke_decrypt(uint8_t *m, uint8_t *sigma, const uint64_t *u,
