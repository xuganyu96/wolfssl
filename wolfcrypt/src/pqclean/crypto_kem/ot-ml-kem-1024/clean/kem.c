 ...
 * Returns 0 (success)
 **************************************************/
int OTMLKEM1024_CLEAN_crypto_kem_keypair_derand(uint8_t *pk, uint8_t *sk,
                                                      const uint8_t *coins) {
...
int OTMLKEM1024_CLEAN_crypto_kem_enc_derand(uint8_t *ct, uint8_t *ss, const uint8_t *pk,
    ...
    OTMLKEM1024_CLEAN_indcpa_enc(ct, buf, pk, kr + KYBER_SYMBYTES);
 ...
 * Returns 0 (success)
 **************************************************/
int OTMLKEM1024_CLEAN_crypto_kem_enc(uint8_t *ct, uint8_t *ss, const uint8_t *pk,
                                           WC_RNG *rng) {
...
int OTMLKEM1024_CLEAN_crypto_kem_dec(uint8_t *ss, const uint8_t *ct, const uint8_t *sk) {
    ...
    OTMLKEM1024_CLEAN_indcpa_dec(buf, ct, sk);
    ...
    /* coins are in kr+KYBER_SYMBYTES */
    OTMLKEM1024_CLEAN_indcpa_enc(cmp, buf, pk, kr + KYBER_SYMBYTES);

    fail = OTMLKEM1024_CLEAN_verify(ct, cmp, KYBER_CIPHERTEXTBYTES);

    /* Compute rejection key */
    rkprf(ss, sk + KYBER_SECRETKEYBYTES - KYBER_SYMBYTES, ct);

    /* Copy true key to return buffer if fail is false */
    OTMLKEM1024_CLEAN_cmov(ss, kr, KYBER_SYMBYTES, (uint8_t)(1 - fail));
