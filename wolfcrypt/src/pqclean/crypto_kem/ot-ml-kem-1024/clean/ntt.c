...
/* Code to generate OTMLKEM1024_CLEAN_zetas and zetas_inv used in the number-theoretic
transform:
...
void init_ntt() {
  ...
  for(i=0;i<128;i++) {
    OTMLKEM1024_CLEAN_zetas[i] = tmp[tree[i]];
    if(OTMLKEM1024_CLEAN_zetas[i] > KYBER_Q/2)
      OTMLKEM1024_CLEAN_zetas[i] -= KYBER_Q;
    if(OTMLKEM1024_CLEAN_zetas[i] < -KYBER_Q/2)
      OTMLKEM1024_CLEAN_zetas[i] += KYBER_Q;
  }
}
...
const int16_t OTMLKEM1024_CLEAN_zetas[128] = {
...
static int16_t fqmul(int16_t a, int16_t b) {
    return OTMLKEM1024_CLEAN_montgomery_reduce((int32_t)a * b);
}

/*************************************************
 * Name:        OTMLKEM1024_CLEAN_ntt
 *
...
void OTMLKEM1024_CLEAN_ntt(int16_t r[256]) {
    ...
    for (len = 128; len >= 2; len >>= 1) {
        for (start = 0; start < 256; start = j + len) {
            zeta = OTMLKEM1024_CLEAN_zetas[k++];
...
void OTMLKEM1024_CLEAN_invntt(int16_t r[256]) {
    ...
    for (len = 2; len <= 128; len <<= 1) {
        for (start = 0; start < 256; start = j + len) {
            ...
            for (j = start; j < start + len; j++) {
                t = r[j];
                r[j] = OTMLKEM1024_CLEAN_barrett_reduce(t + r[j + len]);
                r[j + len] = r[j + len] - t;
...
/*************************************************
 * Name:        OTMLKEM1024_CLEAN_basemul
 *
 ...
 **************************************************/
void OTMLKEM1024_CLEAN_basemul(int16_t r[2], const int16_t a[2], const int16_t b[2],
                                     int16_t zeta) {
    r[0] = fqmul(a[1], b[1]);
    r[0] = fqmul(r[0], zeta);
    r[0] += fqmul(a[0], b[0]);
