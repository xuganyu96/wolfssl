...
#include "wolfssl/wolfcrypt/pqclean/crypto_sign/falcon-1024/clean/inner.h"
...
int PQCLEAN_FALCON1024_CLEAN_crypto_sign_keypair(uint8_t *pk, uint8_t *sk, WC_RNG *wc_rng) {
    ...
    sk[0] = 0x50 + 10;
    u = 1;
    v = PQCLEAN_FALCON1024_CLEAN_trim_i8_encode(sk + u,
    ...
    v = PQCLEAN_FALCON1024_CLEAN_modq_encode(
        pk + 1, PQCLEAN_FALCON1024_CLEAN_CRYPTO_PUBLICKEYBYTES - 1, h, 10);
...
static int do_sign(uint8_t *nonce, uint8_t *sigbuf, size_t *sigbuflen, const uint8_t *m,
    ...
    u = 1;
    v = PQCLEAN_FALCON1024_CLEAN_trim_i8_decode(f, 10, PQCLEAN_FALCON1024_CLEAN_max_fg_bits[10],
                                                sk + u,
                                                PQCLEAN_FALCON1024_CLEAN_CRYPTO_SECRETKEYBYTES - u);
    if (v == 0) {
        return -1;
    ...
    u += v;
    v = PQCLEAN_FALCON1024_CLEAN_trim_i8_decode(g, 10, PQCLEAN_FALCON1024_CLEAN_max_fg_bits[10],
                                                sk + u,
                                                PQCLEAN_FALCON1024_CLEAN_CRYPTO_SECRETKEYBYTES - u);
    if (v == 0) {
        return -1;
...
static int do_verify(const uint8_t *nonce, const uint8_t *sigbuf, size_t sigbuflen,
    ...
    if (PQCLEAN_FALCON1024_CLEAN_modq_decode(h, 10, pk + 1,
