...
/* see api.h */
int PQCLEAN_FALCON512_CLEAN_crypto_sign_keypair(uint8_t *pk, uint8_t *sk, WC_RNG *wc_rng) {
    union {
     ...
     * Generate key pair.
     */
    wc_RNG_GenerateBlock(wc_rng, seed, sizeof(seed));
    ...
    sk[0] = 0x50 + 9;
    u = 1;
    v = PQCLEAN_FALCON512_CLEAN_trim_i8_encode(sk + u,
                                               PQCLEAN_FALCON512_CLEAN_CRYPTO_SECRETKEYBYTES - u, f,
                                               9, PQCLEAN_FALCON512_CLEAN_max_fg_bits[9]);
    if (v == 0) {
        return -1;
    }
    u += v;
    v = PQCLEAN_FALCON512_CLEAN_trim_i8_encode(sk + u,
                                               PQCLEAN_FALCON512_CLEAN_CRYPTO_SECRETKEYBYTES - u, g,
                                               9, PQCLEAN_FALCON512_CLEAN_max_fg_bits[9]);
    if (v == 0) {
        return -1;
    }
    ...
    v = PQCLEAN_FALCON512_CLEAN_modq_encode(
        pk + 1, PQCLEAN_FALCON512_CLEAN_CRYPTO_PUBLICKEYBYTES - 1, h, 9);
...
static int do_sign(uint8_t *nonce, uint8_t *sigbuf, size_t *sigbuflen, const uint8_t *m,
    ...
    u = 1;
    v = PQCLEAN_FALCON512_CLEAN_trim_i8_decode(f, 9, PQCLEAN_FALCON512_CLEAN_max_fg_bits[9], sk + u,
                                               PQCLEAN_FALCON512_CLEAN_CRYPTO_SECRETKEYBYTES - u);
    if (v == 0) {
        return -1;
    }
    u += v;
    v = PQCLEAN_FALCON512_CLEAN_trim_i8_decode(g, 9, PQCLEAN_FALCON512_CLEAN_max_fg_bits[9], sk + u,
                                               PQCLEAN_FALCON512_CLEAN_CRYPTO_SECRETKEYBYTES - u);
    if (v == 0) {
        return -1;
    }
     ...
     */
    wc_RNG_GenerateBlock(wc_rng, nonce, NONCELEN);
...
static int do_verify(const uint8_t *nonce, const uint8_t *sigbuf, size_t sigbuflen,
    ...
    if (PQCLEAN_FALCON512_CLEAN_modq_decode(h, 9, pk + 1,
...
/* see api.h */
int PQCLEAN_FALCON512_CLEAN_crypto_sign_signature(uint8_t *sig, size_t *siglen, const uint8_t *m,
                                                  size_t mlen, const uint8_t *sk, WC_RNG *wc_rng) {
    size_t vlen;

    vlen = PQCLEAN_FALCON512_CLEAN_CRYPTO_BYTES - NONCELEN - 1;
    if (do_sign(sig + 1, sig + 1 + NONCELEN, &vlen, m, mlen, sk, wc_rng) < 0) {
        return -1;
    }
    sig[0] = 0x30 + 9;
...
/* see api.h */
int PQCLEAN_FALCON512_CLEAN_crypto_sign_verify(const uint8_t *sig, size_t siglen, const uint8_t *m,
...
/* see api.h */
int PQCLEAN_FALCON512_CLEAN_crypto_sign(uint8_t *sm, size_t *smlen, const uint8_t *m, size_t mlen,
    ...
    if (do_sign(sm + 2, sigbuf, &sigbuflen, pm, mlen, sk, wc_rng) < 0) {
        return -1;
    }
    pm[mlen] = 0x20 + 9;
    sigbuflen++;
...
/* see api.h */
int PQCLEAN_FALCON512_CLEAN_crypto_sign_open(uint8_t *m, size_t *mlen, const uint8_t *sm,
