...
#include <wolfssl/wolfcrypt/pqclean/crypto_sign/sphincs-shake-128s-simple/clean/utilsx1.h>
...
static void fors_gen_leafx1(unsigned char *leaf, const spx_ctx *ctx, uint32_t addr_idx,
                            void *info) {
...
void fors_pk_from_sig(unsigned char *pk, const unsigned char *sig, const unsigned char *m,
    ...
    for (i = 0; i < SPX_FORS_TREES; i++) {
        ...
        compute_root(roots + (i * SPX_N), leaf, indices[i], idx_offset, sig, SPX_FORS_HEIGHT, ctx,
                     fors_tree_addr);
        sig += SPX_N * SPX_FORS_HEIGHT;
    }
