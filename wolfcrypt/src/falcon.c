...
#include <oqs/oqs.h>
#else
#include <wolfssl/wolfcrypt/pqclean/crypto_sign/falcon-1024/clean/api.h>
#include <wolfssl/wolfcrypt/pqclean/crypto_sign/falcon-512/clean/api.h>
#endif
...
int wc_falcon_make_key(falcon_key *key, WC_RNG *rng) {
    ...
    if (key->level != 1 && key->level != 5) {
        /* TODO: maybe BAD_FUNC_ARG is also viable */
        return SIG_TYPE_E;
    }
    if (key->level == 1) {
...
int wc_falcon_sign_msg(const byte *in, word32 inLen, byte *out, word32 *outLen, falcon_key *key,
...
#endif
    {
        ret = wc_CryptoCb_PqcSign(in, inLen, out, outLen, NULL, 0, WC_HASH_TYPE_NONE, rng,
    ...
    if (*outLen < wc_falcon_sig_size(key)) {
        return BUFFER_E;
    }
    ...
    *outLen = (word32)wide_siglen;
    ret = (pqclean_err == 0) ? 0 : BAD_FUNC_ARG;
#endif
    return ret;
}
 ...
 *          0 otherwise.
 */
int wc_falcon_verify_msg(const byte *sig, word32 sigLen, const byte *msg, word32 msgLen, int *res,
                         falcon_key *key) {
    int ret = 0;
#ifdef HAVE_LIBOQS
    OQS_SIG *oqssig = NULL;
...
#endif
    {
        ret = wc_CryptoCb_PqcVerify(sig, sigLen, msg, msgLen, NULL, 0, WC_HASH_TYPE_NONE, res,
                                    WC_PQC_SIG_TYPE_FALCON, key);
        if (ret != WC_NO_ERR_TRACE(CRYPTOCB_UNAVAILABLE))
            return ret;
        /* fall-through when unavailable */
    ...
    int pqclean_err = 0;
    if (key->level == 1) {
        pqclean_err = PQCLEAN_FALCON512_CLEAN_crypto_sign_verify(sig, sigLen, msg, msgLen, key->p);
        // printf("Falcon-512 siglen %d \n", sigLen);
    } else if (key->level == 5) {
        pqclean_err = PQCLEAN_FALCON1024_CLEAN_crypto_sign_verify(sig, sigLen, msg, msgLen, key->p);
        // printf("Falcon-1024 siglen %d\n", sigLen);
    } /* key level already validated, no need to check else */
    *res = (pqclean_err == 0) ? 1 : 0;
#endif
...
#ifdef WOLF_PRIVATE_KEY_ID
int wc_falcon_init_id(falcon_key *key, const unsigned char *id, int len, void *heap, int devId) {
    int ret = 0;
...
int wc_falcon_init_label(falcon_key *key, const char *label, void *heap, int devId) {
    int ret = 0;
    int labelLen = 0;
 ...
 */
int wc_falcon_import_private_key(const byte *priv, word32 privSz, const byte *pub, word32 pubSz,
                                 falcon_key *key) {
    int ret = 0;
    byte *newPriv = NULL;
    word32 newPrivSz = 0;
    ...
    } else if ((pubSz != FALCON_LEVEL1_PUB_KEY_SIZE) && (pubSz != FALCON_LEVEL5_PUB_KEY_SIZE)) {
        return BAD_FUNC_ARG;
    }
...
int wc_falcon_export_private(falcon_key *key, byte *out, word32 *outLen) {
    ...
    if (key->level == 1) {
        *outLen = FALCON_LEVEL1_PRV_KEY_SIZE;
        ...
        XMEMCPY(out + FALCON_LEVEL1_KEY_SIZE, key->p, FALCON_LEVEL1_PUB_KEY_SIZE);
    } else if (key->level == 5) {
        *outLen = FALCON_LEVEL5_PRV_KEY_SIZE;
        XMEMCPY(out, key->k, FALCON_LEVEL5_KEY_SIZE);
        XMEMCPY(out + FALCON_LEVEL5_KEY_SIZE, key->p, FALCON_LEVEL5_PUB_KEY_SIZE);
    }
 ...
 *         0 otherwise.
 */
int wc_falcon_export_key(falcon_key *key, byte *priv, word32 *privSz, byte *pub, word32 *pubSz) {
    int ret = 0;
...
int wc_falcon_check_key(falcon_key *key) {
    ...
    if (key->level == 1) {
        ret = XMEMCMP(key->p, key->k + FALCON_LEVEL1_KEY_SIZE, FALCON_LEVEL1_PUB_KEY_SIZE);
    } else if (key->level == 5) {
        ret = XMEMCMP(key->p, key->k + FALCON_LEVEL5_KEY_SIZE, FALCON_LEVEL5_PUB_KEY_SIZE);
    }
...
int wc_Falcon_PrivateKeyDecode(const byte *input, word32 *inOutIdx, falcon_key *key, word32 inSz) {
    ...
    if (ret == 0) {
        if (pubKeyLen == 0) {
            ...
            ret = wc_falcon_import_private_key(input, inSz, pubKey, pubKeyLen, key);
        }
    }
    return ret;
}
...
int wc_Falcon_KeyToDer(falcon_key *key, byte *output, word32 inLen) {
    ...
    if (key->level == 1) {
        return SetAsymKeyDer(key->k, FALCON_LEVEL1_KEY_SIZE, key->p, FALCON_LEVEL1_PUB_KEY_SIZE,
                             output, inLen, FALCON_LEVEL1k);
    } else if (key->level == 5) {
        return SetAsymKeyDer(key->k, FALCON_LEVEL5_KEY_SIZE, key->p, FALCON_LEVEL5_PUB_KEY_SIZE,
...
int wc_Falcon_PrivateKeyToDer(falcon_key *key, byte *output, word32 inLen) {
    ...
    if (key->level == 1) {
        return SetAsymKeyDer(key->k, FALCON_LEVEL1_KEY_SIZE, NULL, 0, output, inLen,
                             FALCON_LEVEL1k);
    } else if (key->level == 5) {
        return SetAsymKeyDer(key->k, FALCON_LEVEL5_KEY_SIZE, NULL, 0, output, inLen,
                             FALCON_LEVEL5k);
    }
