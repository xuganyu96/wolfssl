 ...
 * Need to set level (1, 3, 5) and optimization (fast or small) before making key!
 */
int wc_sphincs_make_key(sphincs_key *key, WC_RNG *rng) {
    ...
    /* allocate for maximal seed, but make_key_from_seed might not use all of it*/
 ...
 *          0 otherwise.
 */
int wc_sphincs_sign_msg(const byte *in, word32 inLen, byte *out, word32 *outLen, sphincs_key *key,
                        WC_RNG *rng) {
    int ret = 0;
#ifdef HAVE_LIBOQS
    OQS_SIG *oqssig = NULL;
    ...
    if (ret == 0) {
        if ((key->optim == SPHINCS_FAST_VARIANT) && (key->level == 1)) {
            oqssig = OQS_SIG_new(OQS_SIG_alg_sphincs_shake_128f_simple);
        } else if ((key->optim == SPHINCS_FAST_VARIANT) && (key->level == 3)) {
            oqssig = OQS_SIG_new(OQS_SIG_alg_sphincs_shake_192f_simple);
        } else if ((key->optim == SPHINCS_FAST_VARIANT) && (key->level == 5)) {
            oqssig = OQS_SIG_new(OQS_SIG_alg_sphincs_shake_256f_simple);
        } else if ((key->optim == SPHINCS_SMALL_VARIANT) && (key->level == 1)) {
            oqssig = OQS_SIG_new(OQS_SIG_alg_sphincs_shake_128s_simple);
        } else if ((key->optim == SPHINCS_SMALL_VARIANT) && (key->level == 3)) {
            oqssig = OQS_SIG_new(OQS_SIG_alg_sphincs_shake_192s_simple);
        } else if ((key->optim == SPHINCS_SMALL_VARIANT) && (key->level == 5)) {
    ...
    if ((ret == 0) && (OQS_SIG_sign(oqssig, out, &localOutLen, in, inLen, key->k) == OQS_ERROR)) {
        ret = BAD_FUNC_ARG;
    }
    ...
    if (*outLen < wc_sphincs_sig_size(key)) {
        return BUFFER_E;
    }
 ...
 *          0 otherwise.
 */
int wc_sphincs_verify_msg(const byte *sig, word32 sigLen, const byte *msg, word32 msgLen, int *res,
                          sphincs_key *key) {
    int ret = 0;
#ifdef HAVE_LIBOQS
    OQS_SIG *oqssig = NULL;
    ...
    if ((ret == 0) && (OQS_SIG_verify(oqssig, msg, msgLen, sig, sigLen, key->p) == OQS_ERROR)) {
        ret = SIG_VERIFY_E;
    }
 ...
 * optim [in]   Either SPHINCS_FAST_VARIANT or SPHINCS_SMALL_VARIANT.
...
int wc_sphincs_set_level_and_optim(sphincs_key *key, byte level, byte optim) {
    ...
    if (optim != SPHINCS_FAST_VARIANT && optim != SPHINCS_SMALL_VARIANT) {
        return BAD_FUNC_ARG;
    }
 ...
 * key   [in]  Sphincs key.
 * level [out] The level.
 * optim [out] The optimization variant. SPHINCS_FAST_VARIANT or SPHINCS_SMALL_VARIANT.
...
int wc_sphincs_get_level_and_optim(sphincs_key *key, byte *level, byte *optim) {
    ...
    if (key->optim != SPHINCS_FAST_VARIANT && key->optim != SPHINCS_SMALL_VARIANT) {
        return BAD_FUNC_ARG;
    }
...
static int parse_private_key(const byte *priv, word32 privSz, byte **out, word32 *outSz,
    ...
    *outSz = privSz - idx;
    ...
    if ((key->level == 1) && (*outSz != SPHINCS_LEVEL1_KEY_SIZE + SPHINCS_LEVEL1_PUB_KEY_SIZE)) {
        return BAD_FUNC_ARG;
    } else if ((key->level == 3) &&
               (*outSz != SPHINCS_LEVEL3_KEY_SIZE + SPHINCS_LEVEL3_PUB_KEY_SIZE)) {
        return BAD_FUNC_ARG;
    } else if ((key->level == 5) &&
               (*outSz != SPHINCS_LEVEL5_KEY_SIZE + SPHINCS_LEVEL5_PUB_KEY_SIZE)) {
        return BAD_FUNC_ARG;
    }
 ...
 *         0 otherwise.
 */
int wc_sphincs_import_private_only(const byte *priv, word32 privSz, sphincs_key *key) {
    int ret = 0;
    byte *newPriv = NULL;
    word32 newPrivSz = 0;

    if ((ret = parse_private_key(priv, privSz, &newPriv, &newPrivSz, key)) != 0) {
        return ret;
    }
 ...
 */
int wc_sphincs_import_private_key(const byte *priv, word32 privSz, const byte *pub, word32 pubSz,
                                  sphincs_key *key) {
    int ret = 0;
    byte *newPriv = NULL;
    word32 newPrivSz = 0;
    ...
    } else if ((pubSz != SPHINCS_LEVEL1_PUB_KEY_SIZE) && (pubSz != SPHINCS_LEVEL3_PUB_KEY_SIZE) &&
               (pubSz != SPHINCS_LEVEL5_PUB_KEY_SIZE)) {
        return BAD_FUNC_ARG;
    }
...
int wc_sphincs_export_private(sphincs_key *key, byte *out, word32 *outLen) {
    ...
    if (key->level == 1) {
        *outLen = SPHINCS_LEVEL1_PRV_KEY_SIZE;
        ...
        XMEMCPY(out + SPHINCS_LEVEL1_PRV_KEY_SIZE, key->p, SPHINCS_LEVEL1_PUB_KEY_SIZE);
    } else if (key->level == 3) {
        *outLen = SPHINCS_LEVEL3_PRV_KEY_SIZE;
        XMEMCPY(out, key->k, SPHINCS_LEVEL3_PRV_KEY_SIZE);
        XMEMCPY(out + SPHINCS_LEVEL3_PRV_KEY_SIZE, key->p, SPHINCS_LEVEL3_PUB_KEY_SIZE);
    } else if (key->level == 5) {
        *outLen = SPHINCS_LEVEL5_PRV_KEY_SIZE;
        XMEMCPY(out, key->k, SPHINCS_LEVEL5_PRV_KEY_SIZE);
        XMEMCPY(out + SPHINCS_LEVEL5_PRV_KEY_SIZE, key->p, SPHINCS_LEVEL5_PUB_KEY_SIZE);
    }
 ...
 *         0 otherwise.
 */
int wc_sphincs_export_key(sphincs_key *key, byte *priv, word32 *privSz, byte *pub, word32 *pubSz) {
    int ret = 0;
...
int wc_Sphincs_PrivateKeyDecode(const byte *input, word32 *inOutIdx, sphincs_key *key,
                                word32 inSz) {
    int ret = 0;
    ...
    ret = DecodeAsymKey(input, inOutIdx, inSz, privKey, &privKeyLen, pubKey, &pubKeyLen, keytype);
    if (ret == 0) {
        if (pubKeyLen == 0) {
            ret = wc_sphincs_import_private_only(input, inSz, key);
        } else {
            ret = wc_sphincs_import_private_key(privKey, privKeyLen, pubKey, pubKeyLen, key);
        }
    }
    return ret;
}

int wc_Sphincs_PublicKeyDecode(const byte *input, word32 *inOutIdx, sphincs_key *key, word32 inSz) {
    int ret = 0;
    ...
    ret = DecodeAsymKeyPublic(input, inOutIdx, inSz, pubKey, &pubKeyLen, keytype);
    if (ret == 0) {
 ...
 */
int wc_Sphincs_PublicKeyToDer(sphincs_key *key, byte *output, word32 inLen, int withAlg) {
    int ret;
    byte pubKey[SPHINCS_MAX_PUB_KEY_SIZE];
    word32 pubKeyLen = (word32)sizeof(pubKey);
    int keytype = 0;
    ...
    if (ret == 0) {
        ret = SetAsymKeyDerPublic(pubKey, pubKeyLen, output, inLen, keytype, withAlg);
    }
...
int wc_Sphincs_KeyToDer(sphincs_key *key, byte *output, word32 inLen) {
    ...
    if ((key->level == 1) && (key->optim == SPHINCS_FAST_VARIANT)) {
        return SetAsymKeyDer(key->k, SPHINCS_LEVEL1_KEY_SIZE, key->p, SPHINCS_LEVEL1_KEY_SIZE,
                             output, inLen, SPHINCS_FAST_LEVEL1k);
    } else if ((key->level == 3) && (key->optim == SPHINCS_FAST_VARIANT)) {
        return SetAsymKeyDer(key->k, SPHINCS_LEVEL3_KEY_SIZE, key->p, SPHINCS_LEVEL3_KEY_SIZE,
                             output, inLen, SPHINCS_FAST_LEVEL3k);
    } else if ((key->level == 5) && (key->optim == SPHINCS_FAST_VARIANT)) {
        return SetAsymKeyDer(key->k, SPHINCS_LEVEL5_KEY_SIZE, key->p, SPHINCS_LEVEL5_KEY_SIZE,
...
int wc_Sphincs_PrivateKeyToDer(sphincs_key *key, byte *output, word32 inLen) {
    ...
    if ((key->level == 1) && (key->optim == SPHINCS_FAST_VARIANT)) {
        return SetAsymKeyDer(key->k, SPHINCS_LEVEL1_KEY_SIZE, NULL, 0, output, inLen,
                             SPHINCS_FAST_LEVEL1k);
    } else if ((key->level == 3) && (key->optim == SPHINCS_FAST_VARIANT)) {
        return SetAsymKeyDer(key->k, SPHINCS_LEVEL3_KEY_SIZE, NULL, 0, output, inLen,
                             SPHINCS_FAST_LEVEL3k);
    } else if ((key->level == 5) && (key->optim == SPHINCS_FAST_VARIANT)) {
        return SetAsymKeyDer(key->k, SPHINCS_LEVEL5_KEY_SIZE, NULL, 0, output, inLen,
