...
int wc_sphincs_make_key_from_seed(sphincs_key *key, const byte *seed) {
    ...
    int wc_sphincs_sign_msg(const byte *in, word32 inLen, byte *out, word32 *outLen,
        ...
        if ((ret == 0) &&
            (OQS_SIG_sign(oqssig, out, &localOutLen, in, inLen, key->k) == OQS_ERROR)) {
            ret = BAD_FUNC_ARG;
        }
     ...
     *          0 otherwise.
     */
    int wc_sphincs_verify_msg(const byte *sig, word32 sigLen, const byte *msg, word32 msgLen,
     ...
     */
    int wc_sphincs_import_private_key(const byte *priv, word32 privSz, const byte *pub,
                                      word32 pubSz, sphincs_key *key) {
        int ret = 0;
        byte *newPriv = NULL;
        word32 newPrivSz = 0;
     ...
     *         0 otherwise.
     */
    int wc_sphincs_export_key(sphincs_key * key, byte * priv, word32 * privSz, byte * pub,
                              word32 * pubSz) {
        int ret = 0;
    ...
    int wc_Sphincs_PrivateKeyDecode(const byte *input, word32 *inOutIdx, sphincs_key *key,
        ...
        ret =
            DecodeAsymKey(input, inOutIdx, inSz, privKey, &privKeyLen, pubKey, &pubKeyLen, keytype);
        if (ret == 0) {
            if (pubKeyLen == 0) {
    ...
    int wc_Sphincs_PublicKeyDecode(const byte *input, word32 *inOutIdx, sphincs_key *key,
                                   word32 inSz) {
        int ret = 0;
